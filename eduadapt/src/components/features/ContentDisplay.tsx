...
export function ContentDisplay({ mode, content }: ContentDisplayProps) {
  ...
  const [mapOpen, setMapOpen] = useState(false);  // ← CRÍTICO: controla el modal
  ...
  return (
    <>  {/* ← Fragment necesario para que el modal viva FUERA del div principal */}
          ...
          {mode === 'DYSLEXIA' && (
            ...
            >
              <div className="flex items-center justify-between pr-12">
                <h3 className="text-2xl font-bold text-dark mb-4 flex items-center gap-2 font-dyslexic">
                        ...
                        {q.options.map((opt, j) => {
                          ...
                          return (
                            <button
                              ...
                              disabled={isCorrect}
                              className={`w-full text-left p-4 rounded-xl border-2 transition-all flex items-center justify-between group ${buttonClass}`}
                            >
                              <span>{opt}</span>
      ...
      {/* ── Modal: FUERA del div principal, dentro del Fragment ── */}
      <AnimatePresence>
